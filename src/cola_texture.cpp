...
Image * loadTextureBody(){
    ...
    if (!ImageLoad("texture/cola_texture.bmp", image1)) {
        exit(1);
    }
    return image1;
...
Image * loadTextureStraw(){
    ...
    if (!ImageLoad("texture/straw_texture.bmp", image1)) {
        exit(1);
    }
    return image1;
...
void cola(){
	glPushMatrix();
	glBindTexture(GL_TEXTURE_2D, texture1[6] );
...
void init(void)
{
    ...
    Image *image1 = loadTextureBody();
    if(image1 == NULL){
        printf("Image was not returned from loadTextureBody\n");
        exit(0);
    }

	Image *image2 = loadTextureStraw();
    if(image2 == NULL){
        printf("Image was not returned from loadTextureStraw\n");
        exit(0);
    }
    ...
    glBindTexture(GL_TEXTURE_2D, texture1[2000]);
